...
def duelll(m):
    try:
        ...
        msg = bot.send_message(m.chat.id,
                               m.from_user.first_name + ' хочет сразиться в поиске писюна! Лимит по очкам: ' + str(
                                   limit) + '. Кто готов принять вызов?', reply_markup=kb)
...
def getdickkb(duel):
    ...
    while i <= 9:
        ...
        if i <= 3:
            buttons1.append(
                types.InlineKeyboardButton(text='📦', callback_data='duel?' + callb + '?' + str(duel['number'])))
        elif i <= 6:
            buttons2.append(
                types.InlineKeyboardButton(text='📦', callback_data='duel?' + callb + '?' + str(duel['number'])))
        elif i <= 9:
...
def dd(m):
    try:
            ...
            if chats.find_one({'id': m.chat.id}) == None:
                ...
                if time.time() - t <= 250400:
                    bot.send_message(m.chat.id,
                                     'У бота теперь есть статистика найденных членов - найти её можно по команде /dickstat!')
        count = 0
        for ids in polls:
            pol = polls[ids]
        ...
        while i <= 9:
            ...
            if i <= 3:
                buttons1.append(
                    types.InlineKeyboardButton(text='📦', callback_data=callb + ' ' + str(number) + ' ' + str(randoms)))
            elif i <= 6:
                buttons2.append(
                    types.InlineKeyboardButton(text='📦', callback_data=callb + ' ' + str(number) + ' ' + str(randoms)))
            elif i <= 9:
    ...
    try:
        try:
            ...
            users.update_one({'id': m.reply_to_message.from_user.id},
                             {'$push': {'statuses': m.text.split('/set_status ')[1]}})
            bot.send_message(m.chat.id, 'Успешно!')
        except:
            users.update_one({'id': m.reply_to_message.from_user.id},
                             {'$set': {'statuses': [m.text.split('/set_status ')[1]]}})
            bot.send_message(m.chat.id, 'Успешно!')
    except:
